...
async def main():
    ...
    internal = ",".join(f"'{p}'" for p in INTERNAL_PAYERS)
    async with AsyncSessionLocal() as db:
        q = f"""
            SELECT
                ...
                count(*) FILTER (WHERE payer IN ({internal}))                     AS internal_calls,
                count(*) FILTER (WHERE payer NOT IN ({internal}) OR payer IS NULL) AS external_calls
            FROM x402_payments
        ...
        rows = (await db.execute(text(
            ...
            f"WHERE status='settled' AND payer NOT IN ({internal}) GROUP BY category ORDER BY count(*) DESC"
        ))).fetchall()
        for r in rows:
        ...
        reps = (await db.execute(text(
            ...
            f"WHERE status='settled' AND payer NOT IN ({internal}) "
